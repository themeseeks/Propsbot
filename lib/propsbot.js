...
PropsBot.prototype._sendMessage = function (self, fromUser, originalMessage, toUser, extractedMessage) {
    ...
    fs.readFile('./Get_Workers_Response.xml', function (err, html) {
        if (err) throw err; 
        var document = parse5.parse(html.toString());
        var xhtml = xmlser.serializeToString(document);
        var doc = new dom().parseFromString(xhtml);
        var select = xpath.useNamespaces({"wd": "urn:com.workday/bsvc"});
        var fromEmployeeId = select("//wd:Worker/wd:Worker_Descriptor[contains(.,'" + fromUser.real_name + "')]/../wd:Worker_Reference/wd:ID[@wd:type='Employee_ID']/text()", doc);
        var toEmployeeId = select("//wd:Worker/wd:Worker_Descriptor[contains(.,'" + toUser.real_name + "')]/../wd:Worker_Reference/wd:ID[@wd:type='Employee_ID']/text()", doc);
