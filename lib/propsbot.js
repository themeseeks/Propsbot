...
PropsBot.prototype._sendMessage = function (self, fromUser, originalMessage, toUser, extractedMessage) {
    ...
    fs.readFile('./Get_Workers_Response.xml', 'utf-8', function (err, data) {
        if (err) throw err; 
        // var document = parse5.parse(data.toString());
        // var xml = xmlser.serializeToString(document);
        console.log(data);
        var doc = new dom().parseFromString(data);
        var select = xpath.useNamespaces({"wd": "urn:com.workday/bsvc"});
        var fromEmployeeId = select("//wd:Worker/wd:Worker_Descriptor[contains(.,'" + fromUser.real_name + "')]/../wd:Worker_Reference/wd:ID[@wd:type='Employee_ID']/text()", doc)[0].data;
        var toEmployeeId = select("//wd:Worker/wd:Worker_Descriptor[contains(.,'" + toUser.real_name + "')]/../wd:Worker_Reference/wd:ID[@wd:type='Employee_ID']/text()", doc)[0].data;
    });
    soap.createClient(url, function(err, client) {
    client.setSecurity(wsSecurity);
        client.Give_Feedback(args(extractedMessage), function(err, result) {
            //  console.log(result);
        });
    });
};
